...
def load_train_images(data_dir):
    ...
    for i, filename in enumerate(images_type_A):
        imA = imread(filename,cv2.COLOR_BGR2RGB)
        imB = imread(images_type_B[i],cv2.COLOR_BGR2RGB)
...
def load_test_images(data_dir, num_images):
    ...
    for i in range(len(images_type_A)):
        imA = resize(imread(images_type_A[i],cv2.COLOR_BGR2RGB).astype(np.float32), (128, 128))
        imB = resize(imread(images_type_B[i],cv2.COLOR_BGR2RGB).astype(np.float32), (128, 128))
...
def train():
    ...
    parser.add_argument('--epochs', type=int, default=5, help='Number of epochs that training should run')
    ...
    for epoch in range(epochs):
        ...
        num_batches = (num_batches if num_batches % 2 == 0 else num_batches-1)
        num_batches = int(min(num_batches,100))
