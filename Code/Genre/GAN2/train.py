...
def main():
    ...
    target_real = torch.full((opt.batchSize,), 1.0, dtype=torch.float32, device=device)
    target_fake = torch.full((opt.batchSize,), 0.0, dtype=torch.float32, device=device)
    ...
    experiment_dir = 'logs/tensorboard_data'
    experiment_dir = experiment_dir.replace('/', '_').replace(':', '-')
    print("Tensorboard in  : " + experiment_dir)
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        ...
        if loss_G.item() < 2.5 :
            ...
            torch.save(netD_A.state_dict(), '{}/{}/netD_A.pth'.format(experiment_dir, epoch))
            torch.save(netD_B.state_dict(), '{}/{}/netD_B.pth'.format(experiment_dir, epoch))
        print(" \n ---- Saved in " + os.path.join(experiment_dir, str(epoch)))
